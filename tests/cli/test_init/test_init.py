...
def run_cli(commands: list, cwd=f'./tests/cli'):
    ...
    script_abs = os.path.abspath('./synapsis/deployment/cli.py')
...
def check_requirements(requirements: str, expected_defaults: dict):
    assert('synapsis' in requirements)
...
class TestInit(unittest.TestCase):
    ...
    exchange_defaults = {
        ...
        'ftx': {
            'exchange': 'synapsis.FTX',
            'quote_currency': 'USD'
        },
        'kucoin': {
            'exchange': 'synapsis.Kucoin',
            'quote_currency': 'USDT'
        },
        'oanda': {
            'exchange': 'synapsis.Oanda',
            'quote_currency': 'USD'
        }
    }
    ...
    def test_init(self):
        def parse_init_folder(filepath: str, exchange):
            files = ['backtest.json', 'synapsis.json', 'bot.py', 'keys.json', 'requirements.txt', 'settings.json']
