...
class Strategy:
    ...
    def add_orderbook_event(self, callback: typing.Callable, currency_pair: str):
        ...
        #  required
        self.Schedulers.append(
            Synapsis.Scheduler(self.__orderbook_event_websocket, 1, callback=callback, currency_pair=currency_pair)
        )
    ...
    def backtest(self, asset_id: str, price_data: pd.DataFrame):
        ...
        Args:
            ...
            price_data (dataframe): Price dataframe from synapsis historical download
        """
        ...
        if not self.exchange.get_type() == "paper_trade":
            backtesting_exchange = Synapsis.PaperTrade(backtesting_exchange)

        # Append each of the events the class defines into the backtest
        for i in self.Schedulers:
            i = i  # type: Synapsis.Scheduler
            kwargs = i.get_kwargs()
