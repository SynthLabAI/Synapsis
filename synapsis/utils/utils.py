...
from synapsis.indicators.statistics import min_period, max_period, sum_period
...
def to_synapsis_coin_id(asset_id, exchange, quote_guess=None) -> str:
    if exchange == "binance":
        if quote_guess is not None:
            ...
            for i in quotes:
                if __check_ending(asset_id, i):
                    return to_synapsis_coin_id(asset_id, 'binance', quote_guess=i)
...
def to_exchange_coin_id(synapsis_coin_id, exchange):
    ...
    if exchange == "alpaca":
        return get_base_asset(synapsis_coin_id)
    if exchange == "coinbase_pro":
