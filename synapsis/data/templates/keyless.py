import requests
import synapsis
from synapsis.data import PriceReader
...
def price_event(price, symbol, state: synapsis.StrategyState):
    ...
    if rsi[-1] < 30 and not curr_value:
        ...
        buy = synapsis.trunc(state.interface.cash / price, 5)
...
if __name__ == "__main__":
    ...
    data = requests.get(
        'https://firebasestorage.googleapis.com/v0/b/synapsis-6ada5.appspot.com/o/demo_data.csv?alt=media&token=acfa5c39-8f08-45dc-8be3-2033dc2b7b28').text
    with open('./price_examples.csv', 'w') as file:
        file.write(data)

    # Run on the keyless exchange, starting at 100k
    exchange = synapsis.KeylessExchange(price_reader=PriceReader('./price_examples.csv', 'BTC-USD'))
