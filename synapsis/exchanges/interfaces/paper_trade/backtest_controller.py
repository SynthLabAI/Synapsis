...
from synapsis.exchanges.interfaces.paper_trade.backtest_headers import ABCBacktestController
...
class BackTestController(ABCBacktestController):  # circular import to type model
        ...
        for i in range(len(files)):
            ...
            try:
                ...
                raise IndexError(f"Please remove file {files[i]} in your price caches or "
                                 f"match the synapsis cache format.")
    ...
    def run(self,
        ...
        if self.preferences['settings']['GUI_output']:
            ...
            from synapsis.deployment.cli import is_logged_in
            if is_logged_in():
                try:
                    ...
                    link = f'https://app.synapsis.finance/{project_id}/{model_id}/{platform_result["backtest_id"]}' \
                           f'/backtest'
                    webbrowser.open(
                        link
                    )
