"""
    ...
    Copyright (C) 2021 Synapsis Finance
...
from synapsis.exchanges.orders.stop_loss import StopLossOrder
from synapsis.exchanges.orders.take_profit import TakeProfitOrder
...
class FTXInterface(ExchangeInterface):
    ...
    def get_products(self) -> list:
        ...
        for index, product in enumerate(products):
            if product['type'] == "spot":
                product['symbol'] = utils.to_synapsis_symbol(product.pop('name'), 'ftx')
    ...
    def market_order(self, symbol: str, side: str, size: float) -> MarketOrder:
        ...
        response["symbol"] = utils.to_synapsis_symbol(response.pop("market"), 'ftx')
    ...
    def get_open_orders(self, symbol: str = None) -> list:
        ...
        for open_order in response:
            open_order['symbol'] = utils.to_synapsis_symbol(open_order.pop('market'), 'ftx')
    ...
    def get_product_history(self, symbol, epoch_start, epoch_stop, resolution):
        ...
        Args:
            symbol: synapsis product ID format (BTC-USD)
    ...
    def get_order_filter(self, symbol: str):
        ...
        return {
            "symbol": utils.to_synapsis_symbol(market_info["name"], 'ftx'),  # make function in utils (or static)
