...
from synapsis.frameworks.screener.screener_runner import ScreenerRunner
...
class Screener:
    def __init__(self, exchange: Exchange,
        ...
        Function Signatures:
        init(screener_state: synapsis.ScreenerState)
        evaluator(symbol: str, screener_state: synapsis.ScreenerState)
        final(ScreenerState: synapsis.ScreenerState)
        formatter(raw_results: dict, screener_state: synapsis.ScreenerState)
        ...
        """
        self.resolution = synapsis.utils.time_interval_to_seconds(resolution)

        if not synapsis.is_deployed and synapsis._screener_runner is None:
            synapsis._screener_runner = ScreenerRunner(self.resolution)
        ...
        if synapsis.is_deployed:
            synapsis.reporter.export_screener(self)
    ...
    def __run(self):
        ...
        synapsis.reporter.export_screener_result(self)

    def notify(self, message: str = None):
        """
        Send an email and text message to yourself. When deployed live this will come from an official synapsis email &
