"""
    Updated CLI for interacting with and uploading synapsis models.
...
from synapsis.deployment.ui import text, confirm, print_work, print_failure, print_success, select, show_spinner, path
from synapsis.deployment.exchange_data import EXCHANGES, Exchange, EXCHANGE_CHOICES, exc_display_name, \
    EXCHANGE_CHOICES_NO_KEYLESS
from synapsis.utils.utils import load_deployment_settings, load_user_preferences, load_backtest_preferences
...
def init_starter_model(model):
    path = model.get('path', '.')
    url = model.get('url', 'https://github.com/synapsis-finance/examples')
    ...
    if confirm('Would you like to connect this model to the Synapsis Platform?').unsafe_ask():
        api = ensure_login()
        ensure_model(api)
...
def synapsis_init(args):
    ...
    if exchange:
        ...
        if confirm('Would you like to add keys for this exchange?\n'
                   'You can do this later at any time by running `synapsis key add`').unsafe_ask():
    ...
    with show_spinner('Generating files') as spinner:
        files = [
            ...
            ('requirements.txt', 'synapsis\n', False),
            ('keys.json', generate_keys_json(), True),
            ('synapsis.json', generate_synapsis_json(api, model, model_type), False),
...
def generate_synapsis_json(api: Optional[API], model: Optional[dict], model_type: str, main_script: str = 'bot.py'):
...
def generate_bot_py(exchange: Optional[Exchange], template: str) -> str:
    if not exchange:
        ...
        return pkgutil.get_data('synapsis', 'data/templates/keyless.py').decode('utf-8')
    bot_py = pkgutil.get_data('synapsis', f'data/templates/{template}').decode('utf-8')
...
def ensure_model(api: API):
    ...
    try:
        ...
        print_failure('There was no model detected in this directory. Try running `synapsis init` to create one')
        sys.exit(1)
    ...
    # save model_id and plan back into synapsis.json
    with open('synapsis.json', 'w') as file:
        json.dump(data, file, indent=4)
...
def missing_deployment_files() -> list:
    paths = ['synapsis.json', 'keys.json', 'backtest.json', 'requirements.txt', 'settings.json']
...
def synapsis_key(args):
    func = select('What would you like to do?', [
        Choice('Add a key', synapsis_add_key),
        Choice('Show all keys', synapsis_list_key),
        Choice('Remove a key', synapsis_remove_key)
    ]).unsafe_ask()
    func(args)
...
def main():
    ...
    key_add_parser.set_defaults(func=synapsis_remove_key)
