...
login_parser = subparsers.add_parser('logout', help='Log out of your synapsis account.')
...
def __generate_tempfile():
    """
    Generate a temporary file with the synapsis auth prefix and then get the directory / file name from it
    """
...
def logout():
    ...
    for i_ in os.listdir(temp_folder):
        ...
        if i_[0:13] == 'synapsis_auth_' and i_ != file_name:
...
def main():
    ...
    if which == 'deploy':
        ...
        if 'error' in response:
            ...
            url = f"https://app.synapsis.finance/{api.user_id}/{response['modelId']}/overview"
            webbrowser.open(url)
        ...
        exchange_config = {
            'settings.json': 'https://raw.githubusercontent.com/synapsis-finance/examples/main/configs/settings.json',
        ...
        try:
            if is_logged_in():
                ...
                print(f'{TermColors.WARNING}Run \"synapsis login\" and then \"synapsis init\" again to get better '
                      f'backtest '
                      f'viewing.{TermColors.ENDC}')
        except Exception:
            traceback.print_exc()
            # Wipe the synapsis.json to avoid confusion
        ...
        logout()
        info_print("Cleared all synapsis credentials.")
