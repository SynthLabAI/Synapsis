...
from synapsis.utils.utils import load_json_file, info_print, load_deployment_settings
...
deployment_script_name = 'synapsis.json'
        ...
        if 'model_id' not in deployment_options or \
            ...
            type_ = choose_option('model type', ['strategy', 'screener'],
                                  ["\t" + TermColors.BOLD + TermColors.WARNING +
                                   'A strategy is a model that uses synapsis.Strategy' + TermColors.ENDC,
                                   "\t" + TermColors.BOLD + TermColors.WARNING +
                                   'A screener is a model uses synapsis.Screener' + TermColors.ENDC])

            model_id = api.create_model(project_id, type_, model_name, general_description)['modelId']

            info_print(f"Created a new model in synapsis.json with ID: {model_id}")
...
init_parser = subparsers.add_parser('init', help=f'Sub command to create a synapsis-enabled development environment.'
                                                 f' Supports synapsis init on these exchanges: '
                                                 f'\n{supported_exchanges_str}- Ex: \'synapsis init coinbase_pro\'')
...
login_parser = subparsers.add_parser('login', help='Log in to your synapsis account.')
...
run_parser.add_argument('--monitor',
                        action='store_true',
                        help='Add this flag to monitor the synapsis process to understand CPU & memory usage. '
...
def login(remove_cache: bool = False):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_OPTIONS(self):
            ...
            self.send_header('Access-Control-Allow-Origin', 'https://app.synapsis.finance')
        ...
        def do_GET(self):
            ...
            if 'token' in args and args['token'] is not None:
                ...
                file = requests.get('https://firebasestorage.googleapis.com/v0/b/synapsis-6ada5.appspot.com/o/'
    ...
    url = 'https://app.synapsis.finance/auth/signin?redirectUrl=/deploy'
    webbrowser.open_new(url)

    info_print('Log in on the pop-up window or navigate to: \nhttps://app.synapsis.finance/auth/'
...
def zipdir(path, ziph, ignore_files: list):
    ...
    for root, dirs, files in os.walk(path, topdown=True):
        for file in files:
            # (Modification) Skip everything that is in the synapsis_dist folder
    ...
    if which == 'deploy':
        ...
        if len(projects) == 0:
            print(TermColors.FAIL +
                  "Please create a project with 'synapsis create' first." + TermColors.ENDC)
            return
        ...
        keys_example = requests.get('https://raw.githubusercontent.com/Synapsis-Finance/'
                                    'Synapsis/main/examples/keys_example.json')
        ...
        exchange_config = {
            'settings.json': 'https://raw.githubusercontent.com/synapsis-finance/examples/main/configs/'
                             'settings_sandbox.json',
            'keys.json': 'https://raw.githubusercontent.com/synapsis-finance/examples/main/keys_example.json',
            'backtest_usd.json': 'https://raw.githubusercontent.com/synapsis-finance/examples/main/configs/'
                                 'backtest_usd.json',
            'exchange': user_defined_exchange,
            'bot_url': f'https://raw.githubusercontent.com/synapsis-finance/examples/main/init/rsi_'
        ...
        create_and_write_file('requirements.txt', 'synapsis')
        ...
        #         'https://raw.githubusercontent.com/Synapsis-Finance/Synapsis/main/examples/settings.json')
        #     res = settings.json()
        ...
        #     else:
        #         print("Downloading settings defaults...")
        #     settings = requests.get('https://raw.githubusercontent.com/Synapsis-Finance/'
        #                             'Synapsis/main/examples/settings.json')
        ...
        #     backtest = requests.get('https://raw.githubusercontent.com/Synapsis-Finance/Synapsis/'
        ...
        #     bot = requests.get('https://raw.githubusercontent.com/Synapsis-Finance/Synapsis/main/examples/'
        ...
        if args['path'] is None:
            ...
            deployment_location = os.path.join(
                synapsis_folder, deployment_script_name)
            try:
