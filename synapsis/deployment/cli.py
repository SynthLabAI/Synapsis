...
def main():
    ...
    if which == 'deploy':
        if args['path'] is None:
            ...
            try:
                f = open(os.path.join(args['path'], 'synapsis.json'))
                deployment_options = json.load(f)
            except FileNotFoundError:
                raise FileNotFoundError("A synapsis.json file must be present at the top level of the directory "
                                        "specified.")
        ...
        create_and_write_file('synapsis.json', json.dumps(deploy, indent=2))
        ...
        if args['path'] is None:
            ...
            deployment_location = os.path.join(synapsis_folder, 'synapsis.json')
            try:
